...
```
1、多副本数据冗余
	数据在写入hdfs时，会被切分成一个个的数据块进行存储，每个数据块都会被复制多份分别存储到不同的服务器上。这样的话当某台服务器宕机后，可以从别的服务器上获取相同的数据块。
2、心跳机制
	datanode会定期发送心跳给namenode报备自己的存活状态以及存储块的状态，namenode会根据这些信息维护自己的元数据信息。如果namenode在一段时间间隔内没有接收到datanode发送的心跳，则认为该datanode节点失效，并启动数据再复制机制，直至数据的副本数满足设定要求。
3、namenode高可用
	为了防止单点故障，Hadoop支持主备namenode架构，主节点用于管理元数据，备节点作为备用，当主节点失效，可以迅速进行切换，继续提供服务。
	使用zookeeper监听主备节点，当一段时间没有收到主节点的心跳则认为主节点失效，zookeeper会重新进行选举，然后启用备用节点做主节点继续提供服务。
```

## 31、Hadoop的namenode是如何管理元数据的，这些元数据包括哪些内容

```
1、文件系统命名空间：
	主要包括文件和目录的层次结构，比如文件名、目录名以及他们之间的层次关系。
2、文件块的位置信息：
	记录每个文件被划分成的数据块以及这些数据块存储在哪些datanode节点上。
3、文件属性：
	包括文件的访问权限、所有者及时间戳等信息。
```
