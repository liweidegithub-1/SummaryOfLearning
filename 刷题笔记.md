...
```sql
-- 将（1-5,16,11-13,9）展开为（1,2,3,4,5,16,11,12,13,9）
WITH DATA AS (
...
![image-20251127093815326](D:\Typora\SummaryOfLearning\image\image-20251127093815326.png)

```
从A到B逻辑解析
1、首先需要确定将相同的name拼接到一块，拼接多少次。使用开窗函数count或row_number取最大值都可以计算出相同字符串拼接多少次
2、确定是向上拼接还是向下拼接，本题是向上拼接，也就是说字符串拼接至从上到下第一次出现的位置。使用dense_rank对name进行排序，相同的name排序值一样。
3、过滤出每个name需要拼接的1中计算出来的最大值，使用repeat函数对字符串进行拼接。并且按照2中计算出来的顺序进行排序。
```
...
-- 从A到B
WITH DATA AS (
	SELECT 1 AS ID, 'aa' AS name UNION ALL
	SELECT 2 AS ID, 'aa' AS name UNION ALL
	SELECT 3 AS ID, 'aa' AS name UNION ALL
	SELECT 4 AS ID, 'd' AS name UNION ALL
	SELECT 5 AS ID, 'c' AS name UNION ALL
	SELECT 6 AS ID, 'aa' AS name UNION ALL
	SELECT 7 AS ID, 'aa' AS name UNION ALL
	SELECT 8 AS ID, 'e' AS name UNION ALL
	SELECT 9 AS ID, 'f' AS name UNION ALL
	SELECT 10 AS ID, 'g' AS name
)
SELECT
ID,
SUBSTR(REPEAT(NAME||'|', T1.NUM), 0, LENGTH(REPEAT(NAME||'|', T1.NUM))-1)
FROM (
SELECT
ID,
NAME,
MAX(ID) OVER(PARTITION BY NAME) MAX_ID,
ROW_NUMBER() OVER(PARTITION BY NAME ORDER BY ID) NUM,
DENSE_RANK() over(ORDER BY name) RN
FROM DATA
) T1
WHERE ID = MAX_ID
ORDER BY RN;
...
```
从A到C逻辑
1、首先按照ID进行排序，获取前一行的name值，若是前一行的name和当前行的name相同，则标记为0，若不同则标记为1
2、使用窗口函数按照id进行排序求开始行到当前行的sum值，此时，若是当前行与前一行的name值不同，则sum值会加1，若相同则sum值不变。因此，此处得到的sum值可作为分组条件。
3、按照sum值进行分组，求得分组内最大的ID值，并且将分组内的name值拼接按照ID字段进行排序返回即可得到C
```
...
SELECT
ID, NAME, LAG_NAME,
SUM(IF(NAME=LAG_NAME, 0, 1)) OVER(ORDER BY ID) GROUP_ID
FROM (
SELECT 
ID,
NAME,
LAG(NAME, 1, NAME) OVER(ORDER BY ID) LAG_NAME
FROM DATA
) T1
) T2
...
MAX(ENDDATE),
MIN(STARTDATE),
DATEDIFF(MAX(ENDDATE),MIN(STARTDATE)) + 1 DIFF
FROM (
SELECT
BRAND,
STARTDATE,
ENDDATE,
SUM(FLAG) OVER(PARTITION BY BRAND ORDER BY STARTDATE,ENDDATE) GROUP_ID
FROM (
SELECT
BRAND,
STARTDATE,
ENDDATE,
IF(LAG(ENDDATE,1,STARTDATE) OVER(PARTITION BY BRAND ORDER BY STARTDATE, enddate) >= STARTDATE,0,1) FLAG
FROM DATA
) T1
) T2
...
-- 另一种逻辑，就是把每行数据展开成一列，最后通过品牌名分组，统计天数
-- 1、首先计算每行数据中两个日期的差值，然后生成差值大小的空格，最后使用炸裂函数将这些空格炸裂开，带索引，索引从0开始
-- 2、使用每行数据的开始日期加上索引值，即可将每行数据展开成明细
-- 3、通过对品牌名进行分组，计算明细天数的去重count即可得到每个品牌的活动天数
WITH DATA AS (
	SELECT '华为' AS BRAND, '2018-08-04' AS STARTDATE, '2018-08-05' AS ENDDATE UNION ALL
	SELECT '华为' AS BRAND, '2018-08-04' AS STARTDATE, '2020-12-25' AS ENDDATE UNION ALL
	SELECT '小米' AS BRAND, '2018-08-15' AS STARTDATE, '2018-08-20' AS ENDDATE UNION ALL
...
SELECT
brand,
COUNT(DISTINCT REAL_DATE) AS DAYS FROM (
	SELECT
	BRAND,
	STARTDATE,
	ENDDATE,
...
-- 另一种写法
-- 将原创文章写入数组，然后使用引用文章id判断是否存在于数组中，最后求和
WITH DATA AS (
...
SELECT
OID, SUM(IF(ARRAY_CONTAINS(ARR, OID), 1, 0))
FROM (
SELECT
OID,
COLLECT_LIST(IF(OID=0,ID,NULL)) OVER() ARR
FROM DATA
) T
WHERE OID <> 0
GROUP BY OID;
...
![image-20251128113935415](D:\Typora\SummaryOfLearning\image\image-20251128113935415.png)

```sql
-- 当前金额减去变动总金额的平均值，如果是负数，变为0，多出的部分，顺延到下一行去减
-- 当前金额累加减去平均变动总金额累加
WITH DATA AS (
	SELECT 'A' AS ID, 'A1' AS OID, 10 AS CURR_AMOUNT, 60 AS TOTAL_AMOUNT UNION ALL
	SELECT 'A' AS ID, 'A2' AS OID, 20 AS CURR_AMOUNT, 60 AS TOTAL_AMOUNT UNION ALL
	SELECT 'A' AS ID, 'A3' AS OID, 40 AS CURR_AMOUNT, 60 AS TOTAL_AMOUNT UNION ALL
	SELECT 'B' AS ID, 'B1' AS OID, 5 AS CURR_AMOUNT, 15 AS TOTAL_AMOUNT UNION ALL
	SELECT 'B' AS ID, 'B2' AS OID, 1 AS CURR_AMOUNT, 15 AS TOTAL_AMOUNT UNION ALL
	SELECT 'B' AS ID, 'B3' AS OID, 10 AS CURR_AMOUNT, 15 AS TOTAL_AMOUNT
)
SELECT
ID,
OID,
IF(SUM(CURR_AMOUNT) OVER(PARTITION BY ID ORDER BY OID)-SUM(AVG_AMOUNT) OVER(PARTITION BY ID ORDER BY OID) >= 0, SUM(CURR_AMOUNT) OVER(PARTITION BY ID ORDER BY OID)-SUM(AVG_AMOUNT) OVER(PARTITION BY ID ORDER BY OID), 0) CURR_AMOUNT,
TOTAL_AMOUNT
FROM (
SELECT 
ID,
OID,
CURR_AMOUNT,
TOTAL_AMOUNT/(COUNT(1) OVER(PARTITION BY ID)) AVG_AMOUNT,
TOTAL_AMOUNT
FROM DATA
) T1
```
